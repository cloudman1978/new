...
@section('current')
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-active">
            <a href="/backoffice/admin/indicators/{{$indicator->id}}">Détails de l'indicateur {{$indicator->name}}</a>
        </li>
                        ...
                        <div class="panel-body">
                            <div class="section row" align="center">
                                {!!Form::open(
                                    [
                                        'method' => 'delete',
                                        'route' => ['backoffice.admin.indicators.destroy', $indicator->id]
                                    ])!!}
                                <div class="section">
                                    <div style="width:20%; float:left; line-height: 40px">Nom de l'indicateur: </div>
                                    <h5>{{$indicator->name}}</h5>
                                </div>
                                <div class="section">
                                    <div style="width:20%; float:left; line-height: 40px">Description de l'indicateur: </div>
                                    <h5>{{$indicator->description}}</h5>
                                </div>
                                <div class="section">
                                    <div style="width:20%; float:left; line-height: 40px">Unité de l'indicateur: </div>
                                    <h5>{{$indicator->unity}}</h5>
                                </div>
                                <div class="section">
                                    <div style="width:20%; float:left; line-height: 40px">Valeur usuelle: </div>
                                    <h5>{{$indicator->valUs}}</h5>
                                </div>
                                <div class="section">
                                    <div class="pull-right">
                                        <input  type="submit" value="Supprimer indicateur" id="destroy" class="btn btn-bordered btn-primary" onclick="return confirm(&#39;Vous êtes sur vouloir supprimer ?&#39;);">
                                    </div>
                                </div>
