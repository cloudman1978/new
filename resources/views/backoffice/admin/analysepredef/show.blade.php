...
@section('current')
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-active">
            <a href="/backoffice/admin/analysepredef/{{$analyse->id}}">Détail de l'analyse {{$analyse->name}}</a>
        </li>
                        ...
                        <div class="panel-body">
                            <div class="section row" align="center">
                                {!!Form::open(
                                    [
                                        'method' => 'delete',
                                        'route' => ['backoffice.admin.analysepredef.destroy', $analyse->id]
                                    ])!!}
                                <div class="section">
                                    <div style="width:20%; float:left; line-height: 40px">Nom de l'analyse: </div>
                                    <h5>{{$analyse->name}}</h5><br>
                                </div>
                                <div class="section">
                                    <div style="width:20%; float:left; line-height: 40px">Indicateurs médicaux de l'analyse: </div>
                                    @foreach($analyse->indicators as $ind)
                                    <h5>{{$ind->name}}: {{ $ind->description }}</h5><br>
                                        @endforeach
                                </div>
                                <div class="section">
                                    <div class="pull-right">
                                        <input  type="submit" value="Supprimer analyse prédéfinie" id="destroy" class="btn btn-bordered btn-primary" onclick="return confirm(&#39;Vous êtes sur vouloir supprimer ?&#39;);">
                                    </div>
                                </div>
